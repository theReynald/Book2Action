...
const App: React.FC = () => {
  ...
  const [trendingBooks, setTrendingBooks] = useState<
    Pick<Book, "title" | "author" | "coverImageUrl" | "isbn">[]
  >([]);
  const [isTrendingLoading, setIsTrendingLoading] = useState(false);
  const [searchInputRef, setSearchInputRef] = useState<HTMLInputElement | null>(
    null,
  );
  const [selectedAction, setSelectedAction] = useState<ActionableStep | null>(
    null,
  );
  ...
  const handleSearch = async (title: string, coverImageUrl?: string) => {
    setIsLoading(true);
    setError(null);
    setCurrentBook(null);
    ...
    try {
      ...
      if (result.success && result.book) {
        // Use the known-good cover from typeahead if available
        if (coverImageUrl) {
          result.book.coverImageUrl = coverImageUrl;
        }
      ...
      let devDetails = "";
      if (err) {
        if (err.stack) devDetails += `Stack Trace:\n${err.stack}\n`;
        if (err.message) devDetails += `Message: ${err.message}\n`;
        if (err.response) {
      ...
      setError(
        err && err.message
          ? `Failed to analyze the book: ${err.message}`
    ...
    <div className="min-h-screen py-8 px-4">
      <div className="container mx-auto">
        ...
        {error && (
          <ErrorMessage
            message={error}
            details={(window as any).book2actionErrorDetails || ""}
            onRetry={handleRetry}
          />
        )}
            ...
            <div className="mt-4 text-center overflow-hidden max-w-4xl mx-auto px-6">
              <div className="animated-text-container">
                <p className="animated-text">
                  ...
                  >
                    {currentSearchTitle
                      ? `"${currentSearchTitle.toUpperCase()}"`
                      : "the book"}
                  </span>{" "}
                  to create a custom 7-day action plan and summary for you...
                </p>
              </div>
            </div>
          </div>
