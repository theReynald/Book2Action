...
// API configuration — supports OpenAI direct or OpenRouter
const OPENAI_API_KEY = process.env.REACT_APP_OPENAI_API_KEY || "";
const OPENROUTER_API_KEY = process.env.REACT_APP_OPENROUTER_API_KEY || "";

const useOpenAI = !!OPENAI_API_KEY;
const API_KEY = useOpenAI ? OPENAI_API_KEY : OPENROUTER_API_KEY;
const API_URL = useOpenAI
...
const generateBookAnalysis = async (
  ...
  console.log("- Provider:", useOpenAI ? "OpenAI" : "OpenRouter");
  ...
  if (!API_KEY) {
    console.error(
      "❌ API key not found. Please set REACT_APP_OPENAI_API_KEY or REACT_APP_OPENROUTER_API_KEY in your .env file",
    );
    return null;
  }
...
Requirements:
- If the exact book doesn't exist, find the closest matching real book (e.g. the novelization, tie-in book, companion book, or a book on the same topic) and analyze that instead
- If the query is a movie, TV show, or game title, find the official novelization or a well-known book version of it
- Always return a valid book analysis — never return null or empty
...
Please analyze: "${bookTitle}"`;
    ...
    try {
      ...
      if (bookData.actionableSteps.length !== 7) {
        throw new Error(
          "Expected exactly 7 actionable steps (one for each day of the week)",
        );
      }
      ...
      for (const step of bookData.actionableSteps) {
        if (!step.step || !step.chapter || !step.day) {
          throw new Error(
            "Invalid actionable step format: missing step, chapter, or day",
          );
        }
        ...
        if (
          ...
          !Array.isArray(step.details.sentences) ||
          step.details.sentences.length < 1
        ) {
          step.details.sentences = [
            `This step helps you implement "${step.step}" in your daily life.`,
            `Based on the principles from chapter ${step.chapter}, this action creates lasting change.`,
...
const fallbackBooks: { [key: string]: Book } = {
  ...
  "the 7 habits of highly effective people": {
    ...
    isbn: "9781982137274",
    coverImageUrl:
      "https://books.google.com/books/content?id=ISBN:9781982137274&printsec=frontcover&img=1&zoom=1&source=gbs_api",
    summary:
...
export const searchBook = async (title: string): Promise<BookSearchResult> => {
  ...
  try {
    ...
    console.log(
      `📊 Starting ${useOpenAI ? "OpenAI" : "OpenRouter"} API call...`,
    );
...
const testAPIConnection = async (): Promise<boolean> => {
  try {
    console.log(
      `Testing ${useOpenAI ? "OpenAI" : "OpenRouter"} API connection...`,
    );
        ...
        messages: [
          {
            role: "user",
            content:
              'Hello, this is a test message. Please respond with just "API working".',
          },
        ],
        temperature: 0.7,
        max_tokens: 50,
